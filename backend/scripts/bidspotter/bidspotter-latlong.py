...
def geocode_address(address):
    # Preset default coordinates for the center of Iowa
    default_latitude = 41.8780
    default_longitude = -93.0977
...
# Process the 'bidspotter' table
table_name = 'bidspotter'
logging.info(f"Processing table: {table_name}")
check_and_add_columns(table_name)

# Select the 'location' column from the 'bidspotter' table only if latitude or longitude is NULL
...
for address, latitude, longitude in addresses:
    ...
    if latitude is not None and longitude is not None:
        ...
        connection.commit()
        logging.info(f"Updated table '{table_name}' with Latitude={latitude}, Longitude={longitude} for address '{address}'")
    else:
        logging.warning(f"Geocoding failed for address '{address}' in table '{table_name}'. Latitude and Longitude not updated.")

print("Geocoding and updating completed for the 'bidspotter' table in the auctions database.")
